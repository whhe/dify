...
Usage:  ./setup-mysql-env.sh
                Interactively configure MySQL database parameters to .env file.
...
use_ob=${USE_OCEANBASE:-true}
print_message "info" "Environment variable USE_OCEANBASE is: $use_ob"
...
while [[ $# -gt 0 ]]; do
    ...
    -t | --test)
        ...
        test_connection "$current_db_host" "$current_db_port" "$current_db_user" "$current_db_password" "$current_db_name" "$use_ob" "$current_vector_db_name"
        exit 0
        ;;
    *)
...
USE_OB=$(get_user_input "Use OceanBas?" "$use_ob")
...
test_connection "$DB_HOST" "$DB_PORT" "$DB_USERNAME" "$DB_PASSWORD" "$DB_DATABASE" "$USE_OB" "$OCEANBASE_VECTOR_DATABASE"
